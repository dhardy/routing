...
impl Node {
    ...
    fn get_section_list(&self, prefix: &Prefix<XorName>) -> Result<SectionList, RoutingError> {
        let pub_ids = self.peer_mgr.get_pub_ids(&self.get_section(prefix)?, self.own_id());
        Ok(SectionList::new(*prefix, pub_ids))
    }
    ...
    fn dispatch_routing_message(&mut self,
        ...
        match (routing_msg.content, routing_msg.src, routing_msg.dst) {
            ...
            (ConnectionInfoResponse(conn_info), ManagedNode(src_name), dst @ Client { .. }) |
    ...
    fn handle_candidate_approval(&mut self,
        ...
        let opt_peer_id = match self.route_mgr
            .handle_candidate_approval(&self.peer_mgr, *candidate_id.name(), client_auth) {
        ...
        info!("{:?} Our section with {:?} has approved candidate {}. Adding it to our routing \
              ...
              candidate_id.name(),
              opt_peer_id);
        if self.routing_table().is_merge_in_process() {
    ...
    fn handle_node_approval(&mut self,
        ...
        if let Err(error) = self.route_mgr.add_prefixes(sections.keys().cloned().collect()) {
    ...
    fn add_to_routing_table(&mut self,
        ...
        if self.routing_table().our_section().contains(public_id.name()) {
            self.reset_rt_timer();
        }
    ...
    fn send_section_update(&mut self) {
        if !self.routing_table().is_valid() {
            trace!("{:?} Not sending section update since RT invariant not held.",
                   self);
            return;
        }
        trace!("{:?} Sending section update", self);
        let members = self.peer_mgr.get_pub_ids(self.routing_table().our_section(), self.own_id());
        ...
        let neighbours = self.routing_table().other_prefixes();
    ...
    fn send_connection_info(&mut self,
        ...
        let conn_info = ConnectionInfo {
            encrypted_info: encrypted_conn_info,
            nonce: nonce.0,
            pub_id: *self.full_id().public_id(),
            msg_id: msg_id.unwrap_or_else(MessageId::new),
        };
        let msg_content = if msg_id.is_some() {
            MessageContent::ConnectionInfoResponse(conn_info)
        } else {
            MessageContent::ConnectionInfoRequest(conn_info)
        };
    ...
    fn handle_connection_info_request(&mut self,
                                      ...
                                      -> Result<(), RoutingError> {
        let name = match src {
            Authority::Client { .. } => conn_info.pub_id.name(),
            Authority::ManagedNode(ref name) => name,
            _ => unreachable!(),
        };
        self.routing_table().need_to_add(name)?;
        let their_connection_info = self.decrypt_connection_info(&conn_info.encrypted_info,
                                     &box_::Nonce(conn_info.nonce),
                                     &conn_info.pub_id)?;
        ...
        match self.peer_mgr
            ...
            Ok(IsJoiningNode) => {
                self.send_node_identify(peer_id);
                self.handle_node_identify(conn_info.pub_id, peer_id, outbox);
            }
    ...
    fn handle_connection_info_response(&mut self,
                                       ...
                                       dst: Authority<XorName>)
                                       -> Result<(), RoutingError> {
        self.routing_table().need_to_add(&src)?;
        let their_connection_info = self.decrypt_connection_info(&conn_info.encrypted_info,
                                     &box_::Nonce(conn_info.nonce),
                                     &conn_info.pub_id)?;
        let peer_id = their_connection_info.id();
    ...
    fn handle_section_update(&mut self,
        ...
        while let Some(rt_pfx) = self.routing_table().find_section_prefix(&pfx_name) {
        ...
        let members = members.into_iter()
            .filter(|id: &PublicId| !self.route_mgr.is_expected(id.name()))
            .collect_vec();

        let own_name = *self.name();
        for pub_id in members {
            self.route_mgr.is_expected(pub_id.name());
    ...
    fn handle_rt_rsp(&mut self,
        ...
        for member in members {
            if self.routing_table().need_to_add(member.name()).is_ok() {
    ...
    fn handle_section_split(&mut self, prefix: Prefix<XorName>, outbox: &mut EventBox) {
        ...
        if split_us {
            ...
            for other_pfx in self.routing_table().prefixes() {
        ...
        for new_prefix in vec![prefix.pushed(false), prefix.pushed(true)] {
            if self.routing_table().has_prefix(&new_prefix) {
    ...
    fn handle_own_section_merge(&mut self,
        ...
        let (merge_state, needed_peers) = self.route_mgr
            .merge_own_section(&mut self.peer_mgr, sender_prefix, merge_prefix, sections);
    ...
    fn handle_other_section_merge(&mut self,
                                  ...
                                  outbox: &mut EventBox)
                                  -> Result<(), RoutingError> {
        let needed_peers = self.route_mgr
            .merge_other_section(&mut self.peer_mgr, merge_prefix, section);
        let own_name = *self.name();
    ...
    fn handle_timeout(&mut self, token: u64, outbox: &mut EventBox) -> bool {
        if self.get_approval_timer_token == Some(token) {
            ...
            self.candidate_status_token = Some(self.timer
                .schedule(Duration::from_secs(CANDIDATE_STATUS_INTERVAL_SECS)));
            self.route_mgr.show_candidate_status();
    ...
    fn send_candidate_approval(&mut self) -> Result<(), RoutingError> {
        let (candidate_id, client_auth, sections) = match self.route_mgr
            .verified_candidate_info(&self.peer_mgr) {
            Err(_) => {
    ...
    fn get_targets(&self,
        ...
        let force_via_proxy = match routing_msg.content {
            MessageContent::ConnectionInfoRequest(ref conn_info) |
            MessageContent::ConnectionInfoResponse(ref conn_info) => {
                routing_msg.src.is_client() && conn_info.pub_id == *self.full_id.public_id()
            }
            _ => false,
        };
    ...
    fn send_connection_info_request(&mut self,
        ...
        self.routing_table().need_to_add(&their_name)?;
    ...
    fn dropped_routing_node(&mut self,
        ...
        self.routing_table().find_section_prefix(&details.name).map_or((), |prefix| {
            self.send_section_list_signature(prefix, None);
        });
        if details.was_in_our_section {
            self.reset_rt_timer();
            let our_section_len = self.routing_table().our_section().len();
            self.section_list_sigs.remove_signatures_by(*pub_id, our_section_len);
        }
    ...
    fn send_other_section_merge(&mut self,
        ...
        let section = self.peer_mgr.get_pub_ids(&merge_details.section, self.own_id());
...
impl Node {
    ...
    pub fn clear_state(&mut self) {
        ...
        if self.route_mgr.remove_connecting_peers(&mut self.peer_mgr) {
            self.merge_if_necessary();
        }
    }
    ...
    fn send_routing_message_via_route(&mut self,
        ...
        let sending_names = match routing_msg.src {
            ClientManager(_) | NaeManager(_) | NodeManager(_) | ManagedNode(_) => {
                ...
                let pub_ids = self.peer_mgr.get_pub_ids(section, self.own_id());
                vec![SectionList::new(*self.our_prefix(), pub_ids)]
            }
            Section(_) => {
                vec![SectionList::new(*self.our_prefix(),
                                      self.peer_mgr
                                          .get_pub_ids(self.routing_table()
                                                           .our_section(),
                                                       self.own_id()))]
            }
            PrefixSection(ref prefix) => {
                self.routing_table()
                    ...
                    .filter_map(|(p, members)| if prefix.is_compatible(&p) {
                        Some(SectionList::new(p,
                                              self.peer_mgr.get_pub_ids(&members, self.own_id())))
                    } else {
                        None
                    })
                    .collect()
