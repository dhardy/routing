...
impl PeerManager {
    ...
    /// Updates peer's state to `Candidate` in the peer map.
    ///
    /// Fails with `Err(CandidateIsTunnelling)` if the peer is tunnelling.
    ...
    /// Get a PublicId given a name
    pub fn get_pub_id(&self, name: &XorName) -> Option<&PublicId> {
        self.peer_map.get_by_name(name).map(|peer| peer.pub_id())
    }
    ...
    pub fn get_pub_ids(&self, names: &BTreeSet<XorName>, own_id: &PublicId) -> BTreeSet<PublicId> {
        names.into_iter()
    ...
    /// Handles dropping of nodes during split.
    pub fn drop_via_split(&mut self, names_to_drop: Vec<XorName>) -> Vec<(XorName, PeerId)> {
    ...
    /// Removes the given peer, returning its `PeerId`, if available.
...
impl PeerManager {
    /// Removes all peers that are not connected. Returns `true` if any was removed.
